        ...
        while (true) {
            ...
            try {
                if (parts.length > 1) {
                    ...
                    currData = updateBBoxes($("video-container"), data, { showLabels: true, showUnknown: true });
...
function updateDetectionList(data) {
    ...
    els.sort((a, b) => (a.dataset.name || "").localeCompare(b.dataset.name || ""));
...
// ───────────────────────────── Capture ───────────────────────────────────
captureInput.addEventListener("input", () => (captureBtn.disabled = !hasTarget || !captureInput.value.trim()));
...
captureBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        showCaptureToast(result.message || "Done.", result.ok ? "success" : "error");
...
$("close-references-modal")?.addEventListener("click", () => modal.classList.add("hidden"));
...
function showImages(name = null) {
    ...
    imgCount.textContent = name ? `${total} images for "${name}"` : `${total} images from ${refData.length} people`;
}

nameSelect?.addEventListener("change", (e) => showImages(e.target.value || null));
