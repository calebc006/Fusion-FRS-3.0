...
class FRVidPlayer(VideoPlayer):
    ...
    def __init__(self) -> None:
        ...
        # For interactive capture (unknown target)
    ...
    def _select_unknown_target_index(
        self, labels: list[str], bboxes: list[list[float]]
    ) -> int | None:
        """
        Select the closest unknown face based on largest bbox area.

        Returns the index of the target unknown, or None if none.
        """
    ...
    def _update_capture_target(
        ...
        """
        Store latest frame and target unknown detection for capture.

        Returns the target index or None.
        """

        target_idx = self._select_unknown_target_index(labels, bboxes)

        with self.capture_lock:
            self.latest_frame_bytes = frame_bytes
            if target_idx is not None and target_idx < len(embeddings_list):
                self.latest_target = {
                    "bbox": bboxes[target_idx],
                    "embedding": np.asarray(embeddings_list[target_idx], dtype=np.float32),
    ...
    def _add_embedding_to_index(self, name: str, embedding: np.ndarray) -> None:
        """Add a single embedding to in-memory indices for fast lookup."""

        self.name_list.append(name)
        self.vector_index.add_item(embedding)

        if self.embeddings_array.size == 0:
            self.embeddings_array = np.array([embedding])
        else:
    ...
    def _save_snapshot(self, name: str, frame_bytes: bytes, bbox: list[float]) -> str:
        """Save a cropped face snapshot for the captured person."""
        ...
        left = max(int(bbox[0] * self.width), 0)
        top = max(int(bbox[1] * self.height), 0)
        right = min(int(bbox[2] * self.width), self.width)
        bottom = min(int(bbox[3] * self.height), self.height)
    ...
    def capture_unknown(self, name: str) -> dict:
        """Capture the current target unknown and store it under the provided name."""
        ...
        if not target or not frame_bytes:
            return {"ok": False, "message": "No target unknown face available."}

        embedding = np.asarray(target["embedding"], dtype=np.float32)

        try:
            with get_db() as conn:
                save_record(conn, name, embedding)
            self._add_embedding_to_index(name, embedding)

            snapshot_path = self._save_snapshot(name, frame_bytes, target["bbox"])
            log_info(f"Captured new face for {name}, snapshot: {snapshot_path}")
        except Exception as e:
            log_info(f"Failed to capture face for {name}: {e}")
            return {"ok": False, "message": "Failed to save capture. See logs."}
    ...
    def infer(self, frame_bytes: bytes) -> list[FRResult]:
        ...
        try:
            ...
            if len(self.name_list) == 0:
                # No embeddings loaded: mark all faces as Unknown
                labels = ["Unknown" for _ in faces]
                bboxes = [
                    FRVidPlayer._fractionalise_bbox(
                        width, height, face["bbox"].tolist()
                    )
                    for face in faces
                ]
                distances = [[1.0] for _ in faces]

                target_idx = self._update_capture_target(
                    frame_bytes, embeddings_list, labels, bboxes
                )
                extra_labels = self._update_recent_detections([])
