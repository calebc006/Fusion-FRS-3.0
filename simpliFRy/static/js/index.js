import { getCountryFlag, getDescription, setBBoxPos, clearBBoxes, loadNamelistJSON, delay } from "./utils.js";
...
document.getElementById("init").onsubmit = async (event) => {
    ...
    loadNamelistJSON(namelistPath).then(
      data => {
        namelistJSON = data;
      }
    )
    ...
    fetch(`/start`, {
        ...
        if (data.stream) delay(5000).then(() => {
            startStream(() => alert(`FFMPEG unable to stream from provided source!`))
